...
public class GlobalExceptionHandler {
	...
	public ErrorResponse handleResourceNotFound(ProductNotFoundException exp, HttpServletRequest request) {
		
		return new ErrorResponse(LocalDateTime.now(), HttpStatus.NOT_FOUND.value(), "Not Found",
	...
	public ErrorResponse handleInvalidJson(HttpMessageNotReadableException exp, HttpServletRequest request) {
		...
		return new ErrorResponse(LocalDateTime.now(),
				HttpStatus.BAD_REQUEST.value(),"Bad Request", 
				"Input Data - Json Format Mismatch", request.getRequestURI(),
				null);
